...
export default function AvailabilityManager({ optometristId, optometrists = [], isAdmin }) {
  ...
  useEffect(() => {
    if (optometristId) setSelectedOptometristId(optometristId);
  }, [optometristId]);
  ...
  const activeOptoId = optometristId || selectedOptometristId;

  const scopedRecords = useMemo(
    () => availabilityRecords.filter((r) => r.optometrist_id === activeOptoId),
    [availabilityRecords, activeOptoId]
  );
  ...
  const handleSave = async () => {
    if (!selectedDate || !activeOptoId) return;
    setSaving(true);
    const payload = {
      optometrist_id: activeOptoId,
      date: selectedDate,
      slots: editSlots,
      is_active: editSlots.length > 0,
    };
    ...
    <div className="space-y-6">
      {isAdmin && (
        <div className="space-y-2">
          <Label>אופטומטריסט</Label>
          <Select value={selectedOptometristId} onValueChange={setSelectedOptometristId}>
            <SelectTrigger>
              <SelectValue placeholder="בחרו אופטומטריסט" />
            </SelectTrigger>
            <SelectContent>
              {optometrists.map((o) => (
          ...
          {daysInMonth.map((date) => {
            ...
            return (
              <button
                ...
                disabled={isPast}
                className={`aspect-square flex flex-col items-center justify-center rounded-lg text-sm m-0.5 font-medium relative
      ...
      {selectedDate && (
        <Card className="p-5 space-y-4">
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-3">בחרו שעות פנויות:</p>
          ...
          <Button onClick={handleSave} disabled={saving || !activeOptoId} className="w-full gap-2 rounded-xl">
      ...
      {!selectedDate && (
        <p className="text-center text-muted-foreground text-sm py-4">בחרו תאריך בלוח השנה</p>
      )}
    </div>
  );
}

...
function buildCustomers(appointments) {
  ...
  return Array.from(groups.values())
    .map((customer) => {
      const activeAppointments = customer.appointments.filter(
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter((customer) => {
      const searchable = [
        ...
        customer.phone,
        customer.email,
        ...customer.appointments.map((appointment) => appointment.optometrist_name),
      ].join(" ").toLowerCase();
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="relative max-w-xl">
        ...
        <Input
          ...
          placeholder="חיפוש לפי שם, טלפון, אימייל או אופטומטריסט"
          className="pr-9"
        />
      </div>
        ...
        <div className="grid gap-4">
          {filteredCustomers.map((customer) => {
            ...
            const mailtoUrl = customer.email
              ? `mailto:${customer.email}?subject=${encodeURIComponent("עדכון ממרפאת האופטיקה")}`
              : null;
              ...
              <Card key={customer.key} className="p-5">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0 flex-1 space-y-4">
                    ...
                    <div>
                      <p className="mb-2 text-sm font-semibold">היסטוריית תורים</p>
                      <div className="space-y-2">
                        {customer.appointments.map((appointment) => (
                          <div
                            key={appointment.id}
                            className="grid gap-2 rounded-xl border border-border/60 p-3 text-sm sm:grid-cols-3"
                          >
                            <span className="font-medium">{appointment.optometrist_name || "-"}</span>
